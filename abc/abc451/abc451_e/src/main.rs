...
fn sub(aa: &Vec<Vec<usize>>, unvis: &mut BTreeSet<usize>, vis: &mut BTreeMap<usize, usize>, node: usize) -> bool {
    unvis.remove(&node);
    vis.insert(node, 0);
    let result = if unvis.len() == 0 {
        true
    }
    else {
        let mut found = false;
        let list: Vec<usize> = unvis.iter().copied().collect();
        for nnode in list {
            let w = aa[node][nnode];
            let mut ok = true;
            for (j, ws) in vis.iter_mut() {
                *ws += w;
                if *ws != aa[*j][nnode] {
                    ok = false;
                }
            }
            if ok && sub(aa, unvis, vis, nnode) {
                found = true;
            }
            for (j, ws) in vis.iter_mut() {
                *ws -= w;
