...
fn main() {
    ...
    let s: Vec<char> = io.next_string().chars().collect();
    let t: Vec<char> = io.next_string().chars().collect();
    debug!(s, t);

    let mut d = vec![vec![0; s.len() + 1]; t.len() + 1];
    ...
    let x: usize = d[t.len()].iter().sum();
    let y = (s.len() + 1) * s.len() / 2;
    let z = y - x;
    debug!(x, y, z);
