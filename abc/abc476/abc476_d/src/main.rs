...
fn main() {
    ...
    let x: usize = io.next();
    let y: usize = io.next();
    let mut a: Vec<usize> = io.collect(n);
    let mut b: Vec<usize> = io.collect(m);
    a.sort();
    b.sort();
    ...
    let mut cnt_max = sa.partition_point(|e| *e <= x + k * y);
    ...
    for j in 0..m {
        if b[j] <= k * ry {
            ...
            rx += k * py - b[j];
            //
            let cnt = (j + 1) + sa.partition_point(|e| *e <= rx + k * ry);
