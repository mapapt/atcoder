...
fn main() {
    ...
    for _ in 0..t {
        ...
        let x: Vec<i64> = io.collect(n);
        let y: Vec<i64> = io.collect(n - 1);
        debug!(s, x, y);

        let (mut sc_s, mut sc_r) = if s[0] == 'S' {
            (0, -x[0])
        }
        else {
            (-x[0], 0)
        ...
        for i in 1..n {
            if s[i] == 'S' {
                ...
                let sc_ss = sc_s;
                let sc_rr = sc_r - x[i];
                let sc_rs = sc_r + y[i - 1];
                sc_s = sc_ss.max(sc_rs);
                sc_r = sc_sr.max(sc_rr);
            }
            else {
                let sc_sr = sc_s;
                let sc_ss = sc_s - x[i];
                let sc_rr = sc_r;
                let sc_rs = sc_r - x[i] + y[i - 1];
                sc_s = sc_ss.max(sc_rs);
                sc_r = sc_sr.max(sc_rr);
            }
        }

        io.put(sc_s.max(sc_r));
        io.putn();
    }
}
