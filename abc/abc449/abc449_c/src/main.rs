...
fn main() {
    ...
    let mut v = [const{BTreeSet::new()}; 26];
    for (i, c) in b.iter().enumerate() {
        let j = c - b'a';
        let cnt = v[j as usize].len();
        v[j as usize].insert((i, cnt));
    }
    debug!(v);
    ...
    for u in v {
        for w in u.iter() {
            if let Some(lc) = u.range((w.0 + l, 0)..).next() {
                if let Some(rc) = u.range(..=(w.0 + r, usize::MAX)).last() {
                    let cc = rc.1 + 1 - lc.1;
                    debug!(cc);
                    a += cc;
                }
